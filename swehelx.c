...
static int32 calc_rise_and_set(double tjd_start, int32 ipl, double *dgeo, double *datm, int32 eventflag, int32 helflag, double *trise, char *serr)
{
  ...
  if (!(helflag & SE_HELIACAL_HIGH_PRECISION)) 
...
static int32 my_rise_trans(double tjd, int32 ipl, char* starname, int32 eventtype, int32 helflag, double *dgeo, double *datm, double *tret, char *serr)
{
  ...
  //if (!(helflag & SE_HELIACAL_HIGH_PRECISION) && ipl != -1 && fabs(dgeo[1]) < 58) {
...
static double SunRA(double JDNDaysUT, int32 helflag, char *serr)
{
  ...
  if (1) { //helflag & SE_HELIACAL_HIGH_PRECISION) {
    double x[6];
...
static void default_heliacal_parameters(double *datm, double *dgeo, double *dobs, int helflag)
{
  ...
  if (dobs[1] == 0)
    dobs[1] = 1;
  if (!(helflag & SE_HELIACAL_OPTICAL_PARAMS)) {
    for (i = 2; i <= 5; i++)
      dobs[i] = 0;
  }
...
#if 0
static double VisLimMagn_buggy(double *dobs, double AltO, double AziO, double AltM, double AziM, double JDNDaysUT, double AltS, double AziS, double sunra, double Lat, double HeightEye, double *datm, int32 helflag, int32 *is_scotopic, char *serr)
{
  double C1, C2, Th, kX, Bsk, CorrFactor1, CorrFactor2;
  double log10 = 2.302585092994;
  /*double Age = dobs[0];*/
...
' p [-]
*/
static double p(double Age, double B)
{
  double log10 = 2.302585092994;
  /* age dependancy from Garstang [2000]*/
  return 0.534 - 0.00211 * Age - (0.236 - 0.00127 * Age) * Tanh(0.4 * log(B) / log10 - 2.2);
}

static double VisLimMagn_old(double *dobs, double AltO, double AziO, double AltM, double AziM, double JDNDaysUT, double AltS, double AziS, double sunra, double Lat, double HeightEye, double *datm, int32 helflag, int32 *is_scotopic, char *serr)
{
  double C1, C2, Th, kX, Bsk, Fa;
  double log10 = 2.302585092994;
  double Age = dobs[0];
  double SN = dobs[1];
  Bsk = Bsky(AltO, AziO, AltM, AziM, JDNDaysUT, AltS, AziS, sunra, Lat, HeightEye, datm, helflag, serr);
  ...
  Th = C1 * pow(1 + sqrt(C2 * Bsk), 2) * Fa; /* Th = C1 * (1 + sqrt(C2 * Bsk)) ^ (2) * Fa; */
  kX = Deltam(AltO, AltS, sunra, Lat, HeightEye, datm, helflag, serr);
  /* Visual limiting magnitude of point source*/
  if (SN <= 0.00000001)
    SN = 0.00000001;
...
int32 FAR PASCAL_CONV HeliacalJDut(double JDNDaysUTStart, double Age, double SN, double Lat, double Longitude, double HeightEye, double Temperature, double Pressure, double RH, double VR, char *ObjectName, int TypeEvent, char *AVkind, double *dret, char *serr)
{
  ...
  int32 helflag = SE_HELIACAL_HIGH_PRECISION;
  helflag |= SE_HELIACAL_AVKIND_VR;
  dgeo[0] = Longitude;
  dgeo[1] = Lat;
  dgeo[2] = HeightEye;
  datm[0] = Pressure;
...
static int32 moon_event_arc_vis(double JDNDaysUTStart, double *dgeo, double *datm, double *dobs, int32 TypeEvent, int32 helflag, double *dret, char *serr)
{
  ...
  int32 avkind = helflag & SE_HELIACAL_AVKIND;
  int32 epheflag = helflag & (SEFLG_JPLEPH|SEFLG_SWIEPH|SEFLG_MOSEPH);
  dret[0] = JDNDaysUTStart; /* will be returned in error case */
  if (avkind == 0)
    avkind = SE_HELIACAL_AVKIND_VR;
  if (avkind != SE_HELIACAL_AVKIND_VR) {
    if (serr != NULL)
...
static int32 heliacal_ut_arc_vis(double JDNDaysUTStart, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 TypeEventIn, int32 helflag, double *dret, char *serr_ret)
{
  ...
  double direct, Pressure, Temperature, RH, VR;
  int32 epheflag, retval = OK, d;
  ...
  *serr = '\0';
  ...
  switch(Planet) {
    ...
    default:
      DayStep = 64; maxlength = 320; break;
  }
  DayStep0 = DayStep;
  /* heliacal setting */
  eventtype = TypeEvent;
  ...
  /* normalize the maxlength to the step size */
  {
	...
	if (helflag & SE_HELIACAL_AVKIND_MIN7) sunsangle = -7;
	if (helflag & SE_HELIACAL_AVKIND_MIN9) sunsangle = -9;
...
#if 0
  if (helflag & SE_HELIACAL_AVKIND_VR) {
    double darr[40];
  ...
  if (helflag & SE_HELIACAL_AVKIND_PTO) {
    do {
...
//  printf("tjd0=%f, tjd=%f, dsunpl=%f\n", tjd0, tjd, dsunpl);
  switch (ipl) {
    case SE_MOON: ndays = 6; break;
    case SE_MERCURY: ndays = 50; break;
    case SE_MARS: ndays = 200; break;
    default: ndays = 100; break;
  }
  tfac = 1;
  if (ipl == SE_VENUS) {
    tjd -= 30 * direct_day;
...
//    if (fabs(tret - 2451614.7223061137) < 2)
 //      printf("hier %f\n", vdelta);
...
static int32 time_limit_invisible(double tjd, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, int32 direct, double *tret, char *serr)
{
  ...
  double d, darr[10], phot_scot_opic, phot_scot_opic_sv;
...
static int32 get_heliacal_details(double tday, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 helflag, double *dret, char *serr)
{
  int32 i, retval, direct, TypeEvent;
...
#if 0
  double vl, vl_save, vl_save_save;
  /* find out if there is a second optimum */
  /* is this really necessary ????? */
  for (i = 0; i < 30; i++) {
    tday = dret[1] + i * 2.0 / 1440.0 * direct;
    retval = swe_vis_limit_mag(tday, dgeo, datm, dobs, ObjectName, helflag, darr, serr);
    if (retval == ERR) goto swe_heliacal_err;
    vl = darr[0] - darr[7];
    if (i >= 2 && vl_save > 0 && vl_save_save < vl_save && vl_save > vl) {
      /* second optimum found: determine precise time */
      tday -= 2.0 / 1440.0 * direct;
      retval = time_optimum_visibility(tday, dgeo, datm, dobs, ObjectName, helflag, &tday, serr);
      if (retval == ERR) goto swe_heliacal_err;
      dret[4] = tday;
      /* optimum is in same visibility duration: 
       * take over limits from above */
      if ((tday > dret[0] && tday < dret[2]) || (tday < dret[0] && tday > dret[2])) {
	dret[3] = dret[0];
	dret[4] = dret[2];
      /* optimum is outside the first visibility duration:
       * find visibility limits */
      } else {
	direct *= -1;
	retval = time_limit_invisible(tday, dgeo, datm, dobs, ObjectName, helflag, direct, &tday, serr);
	if (retval == ERR) goto swe_heliacal_err;
	dret[3] = tday;
	direct *= -1;
	retval = time_limit_invisible(tday, dgeo, datm, dobs, ObjectName, helflag, direct, &tday, serr);
...
static int32 heliacal_ut_vis_lim(double tjd_start, double *dgeo, double *datm, double *dobs, char *ObjectName, int32 TypeEventIn, int32 helflag, double *dret, char *serr_ret)
{
  ...
  *serr = '\0';
  ...
  helflag2 = helflag;
  //helflag2 &= ~SE_HELIACAL_HIGH_PRECISION;
  /* 
   * heliacal event
   */
  ...
  //  printf("%f, %f, %f, %d\n", tjd_start, tjd, tjd-tjd_start, retval);
      if (retval != OK)
...
if (1) {
   ...
   */
  if (ipl == SE_MERCURY || ipl == SE_VENUS || TypeEvent <= 2) {
    retval = get_heliacal_details(tday, dgeo, datm, dobs, ObjectName, helflag2, dret, serr);
    if (retval == ERR) goto swe_heliacal_err;
...
int32 FAR PASCAL_CONV swe_heliacal_ut(double JDNDaysUTStart, double *dgeo, double *datm, double *dobs, char *ObjectNameIn, int32 TypeEvent, int32 helflag, double *dret, char *serr_ret)
{
  ...
  char ObjectName[AS_MAXCH], serr[AS_MAXCH];
  double tjd0 = JDNDaysUTStart, dsynperiod, tjdmax, tadd;
  int32 MaxCountSynodicPeriod = MAX_COUNT_SYNPER;
  if (helflag & SE_HELIACAL_LONG_SEARCH)
  ...
  *serr = '\0';
   ...
   */
  if (Planet == SE_MOON) {
    retval = MoonEventJDut(tjd0, dgeo, datm, dobs, TypeEvent, helflag, dret, serr);
    if (serr_ret != NULL && *serr != '\0')
      strcpy(serr_ret, serr);
    return retval;
  }
  ...
  tjdmax = tjd0 + dsynperiod * MaxCountSynodicPeriod * 2;
  tadd = dsynperiod * 0.6;
  ...
  printf("%f <-> %f\n", tadd, dsynperiod * 0.8);
  for (itry = 0; 
       itry < MaxCountSynodicPeriod && retval == -2; 
       itry++, tjd0 += dsynperiod * 0.8) {
    *serr = '\0';
    retval = heliacal_ut(tjd0, dgeo, datm, dobs, ObjectName, TypeEvent, helflag, dret, serr);
