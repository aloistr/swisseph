...
static int32 ev_heliac = 1;
static AS_BOOL have_fixstar = FALSE;
static double datm[4];
static double dobs[6];
static char *plsel = PLSEL_D;
...
int main(int argc, char *argv[])
{
  ...
  double t2, thour = 0;
  double delt;
  datm[0] = 1013; datm[1] = 15; datm[2] = 50; datm[3] = 60;
  dobs[0] = 36; dobs[1] = 1;
...
# endif
  ...
  for (i = 1; i < argc; i++) {
	...
	strcpy(ephepath, argv[i]+5);
      ...
      iflag |= SEFLG_BARYCTR;
    } else if (strncmp(argv[i], "-heliac", 7) == 0) {
      smod |= SMOD_HELIACAL; 
      ev_heliac = 1;
      if (*(argv[i]+7) != '\0')
	ev_heliac = atoi(argv[i] + 7);
    } else if (strncmp(argv[i], "-at", 3) == 0) {
      sscanf(argv[i]+3, "%lf,%lf,%lf,%lf", &(datm[0]), &(datm[1]), &(datm[2]), &(datm[3]));
    } else if (strncmp(argv[i], "-dobs", 5) == 0) {
      sscanf(argv[i]+3, "%lf,%lf", &(dobs[0]), &(dobs[1]));
    } else if (strncmp(argv[i], "-dopt", 5) == 0) {
      sscanf(argv[i]+3, "%lf,%lf,%lf,%lf,%lf,%lf", &(dobs[0]), &(dobs[1]), &(dobs[2]), &(dobs[3]), &(dobs[4]), &(dobs[5]));
...
static int do_heliacal_etc() 
{
  double t_ut, dret[50], tout, x[6];
  int ii, jj, kk, iflagret, helflag = 128|256|2048|512;
  char objnam[AS_MAXCH];
  char *eventnam[] = {"", "morning first", "evening last", "evening first", "morning last"};
  if (have_fixstar && *star != '\0') {
    strcpy(objnam, star);
  } else if (strchr("123456", *plsel)) {
    swe_get_planet_name(letter_to_ipl((int) *plsel), objnam);
    have_fixstar = FALSE;
  } else {
    swe_get_planet_name(SE_VENUS, objnam);
    have_fixstar = FALSE;
  }
  swe_set_topo(geopos[0], geopos[1], geopos[2]); 
  t_ut = tjd;
  ...
  for (ii = 0; ii < nstep; ii++) {
    *sout = '\0';
    iflagret = swe_heliacal_ut(t_ut,geopos,datm,dobs,objnam,ev_heliac,helflag, dret, serr);
    tout = dret[0];
    for (jj = 0; jj < 30; jj++) dret[jj] = 0;
    if (1 && iflagret != ERR && *plsel != '1') {
      iflagret = swe_heliacal_pheno_ut(tout,geopos,datm,dobs,objnam,ev_heliac,helflag, dret, serr);
      if (iflagret != ERR) {
	/* for morning first, evening last or inner planet */
	if (ev_heliac == 1 || ev_heliac == 2 || (strchr("123", *plsel) != NULL)) 
	  tout = dret[12];
      }
    }
    if (*serr != '\0') {
    ...
    if (iflagret == ERR) return ERR;
    swe_calc_ut(tout, SE_SUN, 0, x, serr);
    swe_revjul(tout, SE_GREG_CAL, &jyear, &jmon, &jday, &jut);
    sprintf(sout + strlen(sout), "%s %s:\n  %2d.%2d.%4d greg., %s (%f), sun=%f\n", eventnam[ev_heliac], objnam, jday, jmon, jyear, hms(jut,0), tout, x[0]);
    if (tstep > 0)
      sprintf(sout + strlen(sout), "  durvis=%s, appalt=%f, sunalt=%f, arcv=%f, mag=%f\n", hms(dret[24] * 24, 0), dret[1], dret[4], dret[6], dret[20]);
    do_printf(sout);
    if (ev_heliac == 1 || ev_heliac == 2 || (strchr("23", *plsel) != NULL)) {
      for (jj = 1; jj < tstep; jj++) {
	*sout = '\0';
	for (kk = 0; kk < 30; kk++) dret[kk] = 0;
	tout += 1;
	iflagret = swe_heliacal_pheno_ut(tout,geopos,datm,dobs,objnam,ev_heliac,helflag, dret, serr);
	tout = dret[12];
	iflagret = swe_heliacal_pheno_ut(tout,geopos,datm,dobs,objnam,ev_heliac,helflag, dret, serr);
	tout = dret[12];
	swe_revjul(tout, SE_GREG_CAL, &jyear, &jmon, &jday, &jut);
	sprintf(sout + strlen(sout), "  %2d.%2d.%4d greg., %s (%f)\n", jday, jmon, jyear, hms(jut,0), tout);
	sprintf(sout + strlen(sout), "  durvis=%s, appalt=%f, sunalt=%f, arcv=%f, mag=%f\n", hms(dret[24] * 24, 0), dret[1], dret[4], dret[6], dret[20]);
	do_printf(sout);
      }
    }
      /*sprintf(sout + strlen(sout), "\t%s\t%s", 
                ...
                strcpy(s2, dms(geopos[1], BIT_ROUND_MIN)));*/
    t_ut = tout;
    if (strcmp(objnam, "moon") == 0 || strcmp(objnam, "Moon") == 0)
      t_ut += 25;
    else if (strcmp(objnam, "mercury") == 0 || strcmp(objnam, "Mercury") == 0)
      t_ut += 60;
    else
      t_ut += 300;
  }
