/* SWISSEPH
   $Header: sweht.c,v 1.30 97/07/08 15:16:43 dieter Exp $

  tests the swehouse.c module
  for ASTRODIENST only!!
  more convenient for test than swehtest.c
  epsilon = 23.45 -> polar circle = 66.55
*************************************************/
...
#endif
{
...
#endif
  struct houses h;
  char s[AS_MAXCH], s1[AS_MAXCH], s2[AS_MAXCH];
  char hsy[AS_MAXCH], slat[AS_MAXCH], ssidt[AS_MAXCH];
  double cusp[1+12+3];	/* empty + 12 houses + ac + mc */
  ...
  while (1) {
    ...
    sidt = atof(ssidt);
    armc = atof(ssidt) * 15;
    sprintf (s, "latitude= %f, sid. time= %f, hsy=%c (eps = 23%c27')\n",lat, sidt, hsy[0], ODEGREE_CHAR);
    do_printf(s);
    CalcHouses(armc, lat, 23.45, hsy[0], 1, &h);
    for (i = 1; i <= 12; i++)  {
